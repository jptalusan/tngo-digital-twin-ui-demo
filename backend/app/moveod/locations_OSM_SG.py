...
import os
import math
from app.logging.config import child_get_logger, subprocess_logger_context
import osmnx as ox
...
def process_section(miny, maxy, minx, maxx, tags, log_queue: Queue):
    """Process a geographic section and return building geometries"""
    import time

    logger = child_get_logger(log_queue, __name__)
    ...
    while retries <= max_retries:
        try:
            ...
            logger.error(f"Exception in section {miny:.4f}, {maxy:.4f}, {minx:.4f}, {maxx:.4f}: {type(e).__name__}: {e}")
            retries += 1

    logger.error(f"Failed to process section after {max_retries} retries: {miny:.4f}, {maxy:.4f}, {minx:.4f}, {maxx:.4f}")
    return None
...
class LocationsOSMSG:
    ...
    def find_locations_OSM(self, use_parallel=None):
        ...
        if os.path.exists(buildings_file):
            ...
            with subprocess_logger_context(__name__) as log_queue:
                func_args = [(s[0], s[1], s[2], s[3], {"building": True}, log_queue) for s in splits]
                ...
                if not use_parallel or len(results) == 0:
                    ...
                    result = process_section(miny, maxy, minx, maxx, {"building": True}, log_queue)
