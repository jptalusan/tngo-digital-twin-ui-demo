...
from app.moveod.config import CENSUS_API_KEY
...
def get_states_and_counties(session=None):
    if session is not None:
        from sqlalchemy import select
        from app.models.moveod import CountyFips

        rows = session.execute(
            select(
                CountyFips.state_name,
                CountyFips.state_id,
                CountyFips.county_name,
                CountyFips.county_fips,
                CountyFips.state_fips,
            )
        ).all()

        states: dict = {}
        state_fips_map: dict = {}
        counties_in_state: dict = {}
        county_fips_map: dict = {}

        for row in rows:
            sname = row.state_name
            # 5-digit geoid = state_fips (2) + county_fips (3)
            geoid5 = (row.state_fips or "").zfill(2) + (row.county_fips or "").zfill(3)
            if sname not in states:
                states[sname] = row.state_id or ""
                # state_fips_map stores the numeric state FIPS without leading zero,
                # matching the original CSV behaviour (str(county_fips)[:2] on the 5-digit value)
                state_fips_map[sname] = str(int(row.state_fips)) if row.state_fips else ""
            counties_in_state.setdefault(sname, [])
            if row.county_name not in counties_in_state[sname]:
                counties_in_state[sname].append(row.county_name)
            county_fips_map.setdefault((sname, row.county_name), []).append(geoid5)

        return states, state_fips_map, counties_in_state, county_fips_map
...
def download_lodes(logger, state, state_abbr, lodes_code, year):
    ...
    compressed_path = f"{settings.moveod_output_path}/states/{state}/{state_abbr}_od_main_JT0{lodes_code}_{year}.csv.gz"
    decompressed_path = f"{settings.moveod_output_path}/states/{state}/{state_abbr}_od_main_JT0{lodes_code}_{year}.csv"
    ...
    compressed_path = f"{settings.moveod_output_path}/states/{state}/{state_abbr}_od_aux_JT0{lodes_code}_{year}.csv.gz"
    decompressed_path = f"{settings.moveod_output_path}/states/{state}/{state_abbr}_od_aux_JT0{lodes_code}_{year}.csv"
    download_and_decompress(type, logger, url, compressed_path, decompressed_path)
...
def download_ms_buildings(logger, state, state_stripped):
    ...
    compressed_path = f"{settings.moveod_output_path}/states/{state}/{state_stripped}.geojson.zip"
    decompressed_path = f"{settings.moveod_output_path}/states/{state}/"  # Path to the directory to extract files
    download_and_decompress(type, logger, url, compressed_path, decompressed_path)
